...
blk_his_2016 <- race_2016 %>%
  filter(race == "Blk" | race == "His") %>%
  group_by(major) %>%
  summarize(students_2016 = sum(students)) %>%
  filter(students_2016 != 0)

blk_his_2018 <- race_2018 %>%
  filter(race == "Blk" | race == "His") %>%
  group_by(major) %>%
  summarize(students_2018 = sum(students))

blk_his_2016_2018 <- blk_his_2016 %>%
  left_join(blk_his_2018, by = "major") %>%
...
blk_his_2016_2018 %>%
  ggplot(aes(x = reorder(major, students), y = students, fill = year)) + 
    ...
    labs(title = "Black and Hispanic Sophomore Students Declaring SEAS Concentrations in 2016 \n Compared to Seniors Receiving Degrees from SEAS in 2018",
         subtitle = "Comparing the Retention of Harvard SEAS Black and Hispanic Students from Sophomore to Senior Year",
         x = "Major", y = "Students") + 
    theme_classic() + coord_flip() +
...
female_2016_2018 %>%
  ggplot(aes(x = reorder(major, students), y = students, fill = year)) + 
    geom_bar(position=position_dodge2(reverse = TRUE), stat="identity") +
    labs(title = "Female Sophomore Students Declaring SEAS Concentrations in 2016 \n Compared to Seniors Receiving Degrees from SEAS in 2018",
         subtitle = "Comparing the Retention of Harvard SEAS Female Students from Sophomore to Senior Year",
         x = "Major", y = "Students") + 
    theme_classic() + coord_flip() +
    scale_fill_discrete(name = "Year")
